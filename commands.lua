...
local function createRSpyButtons()
    ...
    for _, btnData in ipairs(buttonNames) do
        ...
        elseif btnData[1] == "Copy Remote" then
            btn.MouseButton1Click:Connect(function()
                if rspySelected then
                    if setclipboard then setclipboard(getRemotePath(rspySelected.remote)) end
            ...
            btn.MouseButton1Click:Connect(function()
                if rspySelected then
                    ...
                    for i = #rspyLogs, 1, -1 do
                        if rspyLogs[i].remote == remote then
...
if rspyFrame then
    local rspyDragbar = rspyFrame:FindFirstChild("RSPY_DRAGBAR")
    local rspyClose = rspyFrame:FindFirstChild("CloseButton")
    
    if rspyDragbar then
        local rspyDragging = false
        local rspyDragStart, rspyStartPos
        ...
        rspyDragbar.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                local rspyDragInput = input
            end
        end)
        
        game:GetService("UserInputService").InputChanged:Connect(function(input)
            if rspyDragging then
